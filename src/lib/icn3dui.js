...
iCn3DUI.prototype.show3DStructure = function (pdbStr) {
    let me = this;
    let thisClass = this;
    me.deferred = $.Deferred(function () {
        if (me.cfg.menuicon) {
            me.htmlCls.wifiStr =
                '<i class="icn3d-wifi" title="requires internet">&nbsp;</i>';
            me.htmlCls.licenseStr =
                '<i class="icn3d-license" title="requires license">&nbsp;</i>';
        } else {
            me.htmlCls.wifiStr = "";
            me.htmlCls.licenseStr = "";
        }
        ...
        if (me.utilsCls.isSessionStorageSupported())
            ic.setStyleCls.getCommandsBeforeCrash();
        ...
        if (me.cfg.usepdbnum !== undefined) {
            ...
            if (me.cfg.date !== undefined) {
                me.icn3d.bUsePdbNum =
                    parseInt(me.cfg.date) >= 20201222 ? true : false;
            } else {
                // iCn3D paper
                if (
                    me.cfg.mmdbid == "1tup" &&
                ...
                //https://link.springer.com/article/10.1007/s00239-020-09934-4/figures/1
                    ...
                    me.cfg.mmdbid == "118496" &&
                    me.cfg.showanno == 0 &&
                    me.cfg.inpara.indexOf("bu=1") != -1
                ) {
                    me.icn3d.bUsePdbNum = false;
                }
                //https://link.springer.com/article/10.1007/s00239-020-09934-4/figures/6
                else if (
                    me.cfg.align == "163605,1,91105,1,1,1" &&
                    me.cfg.inpara.indexOf("atype=1") != -1
                ) {
                    me.icn3d.bUsePdbNum = false;
                } else {
        ...
        if (me.cfg.controlGl) {
            ic.bControlGl = true;
            ic.container =
                ic.bControlGl && !me.bNode ? $(document) : $("#" + ic.id);
        }
        ...
        if (me.cfg.bCalphaOnly !== undefined)
            ic.bCalphaOnly = me.cfg.bCalphaOnly;
        ...
        if (pdbStr) {
            ...
            if (me.cfg.resdef !== undefined && me.cfg.chains !== undefined) {
                ...
                if (structureArray.length == chainArray.length) {
                    for (let i = 0, il = structureArray.length; i < il; ++i) {
                        chainidArray.push(
                            structureArray[i] + "_" + chainArray[i]
                        );
                    }

                    let bRealign = true,
                        bPredefined = true;
                    ic.realignParserCls.realignChainOnSeqAlign(
                        undefined,
                        chainidArray,
                        bRealign,
                        bPredefined
                ...
                me.cfg.resdef !== undefined &&
                me.cfg.matchedchains !== undefined
            ) {
                ...
                for (let i = 0, il = chainidArray.length; i < il; ++i) {
                    if (i > 0) mmdbafid += ",";
                    mmdbafid += chainidArray[i].substr(
                        0,
                        chainidArray[i].indexOf("_")
                    );
                }
            ...
            ic.bNCBI = true;
            ic.loadCmd =
                "load mmdb " + me.cfg.mmdbid + " | parameters " + me.cfg.inpara;
            me.htmlCls.clickMenuCls.setLogCmd(ic.loadCmd, true);
            ...
            if (
                me.cfg.query_id.substr(0, 5) !== "Query" &&
                me.cfg.rid === undefined
            ) {
                ...
                me.htmlCls.clickMenuCls.setLogCmd(ic.loadCmd, true);
                ic.mmdbParserCls.downloadBlast_rep_id(
                    me.cfg.query_id + "," + me.cfg.blast_rep_id
                );
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        for (
                            let q = 0, ql = data.BlastOutput2.length;
                            q < ql;
                            ++q
                        ) {
                            if (
                                data.BlastOutput2[q].report.results.search
                                    .query_id != me.cfg.query_id
                            )
                                continue;
                            let hitArray =
                                data.BlastOutput2[q].report.results.search.hits;
                            let qseq = undefined;
                            for (let i = 0, il = hitArray.length; i < il; ++i) {
                                let hit = hitArray[i];
                                let bFound = false;
                                for (
                                    let j = 0, jl = hit.description.length;
                                    j < jl;
                                    ++j
                                ) {
                            ...
                            ic.inputid =
                                me.cfg.query_id + "_" + me.cfg.blast_rep_id;
                            ic.loadCmd =
                                "load seq_struct_ids " +
                                me.cfg.query_id +
                                "," +
                    ...
                    error: function (xhr, textStatus, errorThrown) {
                        ...
                        if (this.tryCount <= this.retryLimit) {
                            ...
                            alert(
                                "The RID " + me.cfg.rid + " may have expired..."
                            );
                        }
                        return;
                    },
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (
                        data.InformationList !== undefined &&
                        data.InformationList.Information !== undefined
                    )
            ...
            ic.loadCmd =
                ...
                me.cfg.inpara;
            me.htmlCls.clickMenuCls.setLogCmd(ic.loadCmd, true);
            ic.chainalignParserCls.downloadChainalignment(
                me.cfg.chainalign,
                me.cfg.resnum,
                me.cfg.resdef
            );
            ...
            $.when(ic.chainalignParserCls.downloadMmdbAf(me.cfg.mmdbafid)).then(
                function () {
...
iCn3DUI.prototype.setDialogAjax = function () {
    ...
    if (!me.bNode && !$.ui.dialog.prototype._makeDraggableBase) {
        $.ui.dialog.prototype._makeDraggableBase =
            $.ui.dialog.prototype._makeDraggable;
