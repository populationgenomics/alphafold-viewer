...
export type Schema = typeof SchemaVar;
...
export function getConfidenceScore(e: StructureElement.Location) {
    ...
    return prop.data.score.has(rI)
        ? prop.data.score.get(rI)!
        : [-1, "No Score"];
}
...
> = CustomModelProperty.createProvider({
    ...
    type: "static",
    defaultParams: AfConfidenceParamsVar,
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    getParams: (data: Model) => AfConfidenceParamsVar,
    ...
    obtain: async (
        ...
        const p = { ...PD.getDefaultValues(AfConfidenceParamsVar), ...props };
...
function createScoreMapFromCif(
    modelData: Model,
    residueData: Table<typeof SchemaVar.local_metric_values>
): AfConfidence["data"] | undefined {
    const ret = new Map<ResidueIndex, [number, string]>();
    const { label_asym_id, label_seq_id, metric_value, _rowCount } =
        residueData;
