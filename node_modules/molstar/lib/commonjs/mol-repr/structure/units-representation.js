...
function UnitsRepresentation(label, ctx, getParams, visualCtor) {
    ...
    function setState(state) {
        ...
        if (overpaint !== undefined && !overpaint_1.Overpaint.areEqual(overpaint, _state.overpaint)) {
            if (_structure) {
                newState.overpaint = overpaint_1.Overpaint.remap(overpaint, _structure);
            }
        }
        if (transparency !== undefined && !transparency_1.Transparency.areEqual(transparency, _state.transparency)) {
            if (_structure) {
                newState.transparency = transparency_1.Transparency.remap(transparency, _structure);
            }
        }
        if (clipping !== undefined && !clipping_1.Clipping.areEqual(clipping, _state.clipping)) {
            if (_structure) {
